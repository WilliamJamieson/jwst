...
class ResidualFringeCorrection():
    ...
    def do_correction(self):
        ...
        self.stat_table = Table(names=('Slice', 'mean', 'median', 'stddev', 'max', 'pmean', 'pmedian', 'pstddev', 'pmax'),
                                dtype=('i4', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8', 'f8'))

        self.out_table = Table(names=('Slice', 'col', 'fringe', 'sn', 'pre_contrast', 'post_contrast',
        ...
        # intermediate output
        n_wav_samples = 1024
        self.qual_table = Table(names=('column', 'quality'),
                                units=('', ''),
        ...
        self.output_data *= self.normalization_factor
        self.model.data = self.output_data
        ...
        if self.save_intermediate_results:
            ...
            ascii.write(self.stat_table, stat_table_name, format='ecsv',
            ...
            ascii.write(self.out_table, out_table_name, format='ecsv',
                        fast_writer=False, overwrite=True)

            t = fits.BinTableHDU(data=self.qual_table, name='FIT_QUAL')

            fit_results_name = self.make_output_path(
                basepath=self.input_model.meta.filename,
                suffix='fit_results', ext='.fits')
            log.info('Saving intermediate fit results output {}'.format(
                fit_results_name))
            ...
            hdu4 = fits.ImageHDU(self.weighted_pix_num,
                                 name='WEIGHTED_PIXEL_FRACTION')
    ...
    def process_channel(self, c):
        ...
        (slices_in_band, xrange_channel, slice_x_ranges, all_slice_masks) = \
            utils.slice_info(self.slice_map, c)
        ...
        for n, ss in enumerate(slices_in_band):
            correction_quality = []
            self.process_slice(n, ss, ysize, xsize, y, x,
                               wave_map, slice_x_ranges, all_slice_masks, correction_quality)
        ...
        log.info(
            'Number of columns corrected for channel {}'.format(self.num_corrected))
    ...
    def process_slice(self, n, ss, ysize, xsize, y, x, wave_map, slice_x_ranges,
        ...
        print('n = ', n)
        log.debug(" X ranges of slice {} {}".format(
        ...
        this_row = np.where(
            self.freq_table['slice'] == float(ss))[0][0]
        log.debug('Row in reference file for slice {}'.format(this_row))

        slice_row = self.freq_table[(
            self.freq_table['slice'] == float(ss))]
        ...
        for col in np.arange(slice_x_ranges[n, 1], slice_x_ranges[n, 2]):
            self.process_column(col, ss, ss_data, ss_wmap, ss_weight, ffreq, dffreq, min_snr,
                                min_nfringes, max_nfringes, pgram_res, correction_quality)

    def process_column(self, col, ss, ss_data, ss_wmap, ss_weight, ffreq, dffreq, min_snr,
                       min_nfringes, max_nfringes, pgram_res, correction_quality):
        ...
        if num_good > 50:
            ...
            if (snr2 < min_snr[0]):
                log.debug('SNR too high not fitting column {}, {}, {}'.format(
                    col, snr2, min_snr[0]))
                pass
            else:
                ...
                try:
                    for fn, ff in enumerate(ffreq):
                        ...
                        result = self.remove_fringe(col, fn, ff, ffreq, dffreq, min_snr, snr2, ss,
                                                    min_nfringes, max_nfringes, pgram_res, proc_data, proc_factors, pre_contrast, bg_fit, res_fringes, res_fringe_fit,
                                                    res_fringe_fit_flag, wpix_num, col_wnum, col_weight, col_max_amp,
                                                    weights_feat)
                        if result:
                    ...
                    fit_res = np.divide(fringe_sub, pbg_fit, out=np.zeros_like(fringe_sub),
                                        where=pbg_fit != 0)
                    fit_res = np.subtract(
                        fit_res, 1, where=fit_res != 0)
                    fit_res *= np.where(col_weight >
                                        1e-07, 1, 1e-08)
                    ...
                    self.out_table.add_row((ss, col, fn, snr2, pre_contrast, contrast, pgram_res[0],
                                            opt_nfringe, peak_freq, freq_min, freq_max))

                    self.qual_table.add_row((col, quality))
                    correction_quality.append(
                        [contrast, pre_contrast])
                    log.debug(
                        " residual contrast = {}".format(contrast))

                    # replace the corrected in-slice column pixels in the data_cor array
                    log.debug(
                        " updating the trace pixels in the output")
                    ...
                    self.weighted_pix_num[idx, col] = np.ones(
                        1024)[idx] * (wpix_num / 1024)
                    self.rejected_fit[idx,
                                      col] = res_fringe_fit_flag[idx]
                    self.background_fit[idx, col] = bg_fit[idx]
                    self.knot_locations[:bgindx.shape[0],
                                        col] = bgindx
    ...
    def remove_fringe(self, col, fn, ff, ffreq, dffreq, min_snr, snr2, ss,
        ...
        if ff > 1e-03:
            ...
            if snr2 > min_snr[fn]:
                ...
                proc_data = np.nan_to_num(
                    proc_data)
                self.out_table.add_row((ss, col, fn, snr2, pre_contrast, pre_contrast, pgram_res[fn],
                                        opt_nfringe, peak_freq, freq_min, freq_max))
            return opt_nfringe, peak_freq, freq_min, freq_max, bgindx
        return None

    def normalize_output_data(self):
        # normalise the output_data to remove units
        self.output_data = self.model.data.copy()
        pos_data = self.input_model.data[self.input_model.data > 0]
        self.normalization_factor = np.median(pos_data)
        self.output_data /= self.normalization_factor

    def load_fringe_table(self):
        residual_fringe_model = datamodels.ResidualFringeModel(
    ...
    def load_regions(self):
        ...
        slice_map = (allregions.regions)[
            self.transmission_level - 1, :, :].copy()
        log.info(" Using {} throughput threshhold.".format(
